...
// get bed types from DB
$bedTypesInitialData = htmlspecialchars(json_encode(getAllBedTypes()), ENT_QUOTES, 'UTF-8');
...
if(isset($_POST['form_identifier'])){
    ...
    }elseif($identifier == 'add_new_room'){
    ...
    }elseif($identifier == 'manage_bed_types'){
        $bedTypes = isset($_POST["bedTypes"]) ? $_POST["bedTypes"] : '[]';
        // var_dump($bedTypes);
        // die();
        $status = updateBedTypes(json_decode($bedTypes, true));
        if($status[0]){
            header("Location: roomsManager.php");
        }
    };
...
<nav id="navBar" class="fixed select-none flex bg-gradient-to-b duration-1000 transition-transform ease-in-out from-brand-black via-brand-black/90 to-transparent h-[6rem] w-full z-20 top-0 left-0 px-[10%]">
    ...
    <div class="rightNav w-full text-white flex justify-end items-center gap-5 text-center">
        ...
        <!-- Manage Bed Types -->
        <button id="manageBedTypesBtn" class="border-2 border-brand-gold px-4 py-1 rounded-md text-brand-gold text-[1rem]">Manage Bed Types</button>
        
        <!-- Add New Room Button -->
        <button id="addNewRoomBtn" class="border-2 border-brand-gold px-4 py-1 rounded-md text-brand-gold text-[1rem]">Add New Room</button>
    ...
    <div id="addNewRoomMenu" class="hidden fixed w-dvw h-dvh top-0 left-0 z-30 items-center justify-center bg-brand-black/40 backdrop-blur-sm" data-initial-bed-types="<?= $bedTypesInitialData?>">
        <form method="post" enctype="multipart/form-data" id="innerAddNewRoom" class="w-[45rem] h-fit bg-brand-black border-2 border-brand-gold rounded-lg text-brand-gold py-5 px-[3rem]">
            ...
            <div id="priceBox" class="inputContainer mb-3 flex justify-between w-full items-center">
                <label for="price" class="text-[1.5rem]">Price</label>
                <input name="price" type="number" id="price" placeholder="e.g. 500000" class="placeholder:text-gray-500 px-4 py-2 w-[60%] bg-brand-black hover:bg-brand-gold text-brand-gold duration-200 hover:text-brand-black rounded-full border-2 border-brand-gold text-[1rem] focus:outline-none">
            </div>
            ...
            <div id="facilityInputComponent" data-initial-facilities='[{"id":1,"name":"WiFi"},{"id":2,"name":"AC"},{"id":3,"name":"Smart TV"}]'>
                <div class="inputContainer mb-4 flex justify-between items-center w-full">
                    <label for="facilityInput" class="text-[1.5rem] text-brand-gold whitespace-nowrap pr-4">Facility</label>
                    <div class="flex items-center w-[70%] md:w-[60%] space-x-2">
                        <input type="text" id="facilityInput" placeholder="enter a facility" class="flex-grow w-full bg-brand-black text-brand-gold placeholder:text-gray-500 px-4 py-2 rounded-full border-2 border-brand-gold hover:bg-brand-gold hover:text-brand-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-black focus:ring-brand-gold transition-colors duration-200">
                        <button id="addFacilityBtn" class="flex-shrink-0 bg-brand-gold hover:bg-opacity-80 text-brand-black rounded-full p-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-black focus:ring-brand-gold transition-colors duration-200" aria-label="Add facility" type="button">
                ...
                <input type="hidden" id="facilitiesInput" name="facilities">
                 <p class="text-sm text-gray-400 pl-[30%] md:pl-[40%] mt-2 -mb-1">Click a tag's text to edit.</p>
                <div id="facilityTagsContainer" class="flex flex-wrap gap-2 pl-[30%] md:pl-[40%] mt-2"></div>
            </div>

            <div class="row flex justify-evenly mt-7 ">
                <button id="cancelAddNewRoomBtn" type="button" class=" duration-200 text-[1.3rem] border-2 border-red-600 px-6 py-1 rounded-lg text-red-600 hover:bg-red-600 hover:text-brand-black">Cancel</button>
                <button id="saveNewRoomBtn" type="submit" class=" duration-200 text-[1.3rem] border-2 border-brand-gold px-6 py-1 rounded-lg text-brand-gold hover:bg-brand-gold hover:text-brand-black">Save</button>
            </div>
        </form>
    </div>

    <!-- POPUP MENU FOR MANAGE BED TYPE -->
    <div id="manageBedTypesMenu" class="hidden fixed w-dvw h-dvh top-0 left-0 z-30 items-center justify-center bg-brand-black/40 backdrop-blur-sm" data-initial-bed-types='<?= $bedTypesInitialData?>'>
        <form method="post" id="innerBedTypesMenu" class="w-[45rem] h-fit bg-brand-black border-2 border-brand-gold rounded-lg text-brand-gold py-5 px-[3rem]">
            <input type="hidden" name="form_identifier" value="manage_bed_types">
            <h3 class="text-center mb-4 text-[2.5rem]">Manage Bed Types</h3>
            <div id="bedTypesMenuComponent">
                <div class="inputContainer mb-4 flex justify-between items-center w-full">
                    <label for="bedTypeInput" class="text-[1.5rem] text-brand-gold whitespace-nowrap pr-4">Bed Types</label>
                    <div class="flex items-center w-[70%] md:w-[60%] space-x-2">
                        <input type="text" id="bedTypeInput" placeholder="enter a bed type" class="flex-grow w-full bg-brand-black text-brand-gold placeholder:text-gray-500 px-4 py-2 rounded-full border-2 border-brand-gold hover:bg-brand-gold hover:text-brand-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-black focus:ring-brand-gold transition-colors duration-200">
                        <button id="addBedTypeBtn" class="flex-shrink-0 bg-brand-gold hover:bg-opacity-80 text-brand-black rounded-full p-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-black focus:ring-brand-gold transition-colors duration-200" aria-label="Add bed type" type="button">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                        </button>
                    </div>
                </div>
            ...
            <div class="row flex justify-evenly mt-7 ">
                ...
                <button id="saveBedTypesBtn" type="submit" class=" duration-200 text-[1.3rem] border-2 border-brand-gold px-6 py-1 rounded-lg text-brand-gold hover:bg-brand-gold hover:text-brand-black">Save</button>
            </div>
        </form>
    </div>
...
<!-- VIEW LOGIC: MANAGE BED TYPES-->
<script>
    manageBedTypesMenuIsOpen = false;
    manageBedTypesMenu = document.querySelector("#manageBedTypesMenu");
    classListManageBedTypes = manageBedTypesMenu.classList;
    
    $("#manageBedTypesBtn").click(function(e){
        e.stopPropagation();
        if(manageBedTypesMenuIsOpen){
            closeManageBedTypesMenu();
        }else{
            bedTypeManager.initialize();
            classListManageBedTypes.remove('hidden')
            classListManageBedTypes.add('flex');
            manageBedTypesMenuIsOpen = true;
        }
    });
    
    $("#innerManageBedTypesMenu").click(function(e){
        e.stopPropagation();
    });
    ...
    $("#cancelManageBedTypesBtn").click(function(){
        closeManageBedTypesMenu();
    });
    
    function closeManageBedTypesMenu(){
        classListManageBedTypes.add('hidden')
        classListManageBedTypes.remove('flex'); // hide menu if open
        manageBedTypesMenuIsOpen = false;
    }
</script>
...
<!-- TAG MANAGER -->
<script src="script/createTagManager.js"></script>

<!-- INTANTIATE TAG MANAGE FOR BED TYPE AND FACILITY -->
<script>
    const bedTypeManager = createTagManager({
        componentId: 'manageBedTypesMenu',
        inputId: 'bedTypeInput',
        addBtnId: 'addBedTypeBtn',
        tagsContainerId: 'bedTypeTagsContainer',
        hiddenInputId: 'bedTypesInputResult',
        initialDataKey: 'initialBedTypes', // Corresponds to data-initial-bed-types
        newIdPrefix: 'new_bedtype'
    });

    const facilityManager = createTagManager({
        componentId: 'facilityInputComponent',
        inputId: 'facilityInput',
        addBtnId: 'addFacilityBtn',
        tagsContainerId: 'facilityTagsContainer',
        hiddenInputId: 'facilitiesInput',
        initialDataKey: 'initialFacilities', // Corresponds to data-initial-facilities
